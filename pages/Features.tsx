        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8" aria-label="Key Features">
          {FEATURES.map((feature, i) => (
            <motion.div
              ...
              className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 p-12 rounded-[40px] group hover:scale-[1.02] transition-all shadow-xl dark:shadow-none"
            >
              <div className="flex justify-between items-start mb-8">
                ...
                <span className="text-4xl font-display font-black text-gray-200 dark:text-white/20 group-hover:text-brand-magenta/40 transition-colors">
                  {feature.metric}
                </span>
              </div>
              <h3 className="text-3xl font-display font-bold mb-4 text-gray-900 dark:text-white transition-colors">{feature.title}</h3>
              <p className="text-gray-500 dark:text-white/40 text-lg leading-relaxed transition-colors">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
